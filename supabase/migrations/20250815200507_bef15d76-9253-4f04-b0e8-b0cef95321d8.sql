...
INSERT INTO public.module_packages (code, name, description, target_audience, module_ids, base_price_monthly, discount_percentage, is_featured) VALUES
('restaurant_only', 'Restaurant Only', 'Solution complète pour restaurants', 'Restaurants', 
 (SELECT ARRAY[id] FROM public.modules WHERE code IN ('core', 'restaurant')), 68, 0, true),
 
('hotel_basic', 'Hôtel Basique', 'Gestion hôtelière essentielle', 'Petits hôtels', 
 (SELECT ARRAY[id] FROM public.modules WHERE code IN ('core', 'hotel')), 78, 0, true),
 
('hotel_complete', 'Hôtel Complet', 'Hôtel avec gestion opérationnelle', 'Hôtels moyens', 
 (SELECT ARRAY[id] FROM public.modules WHERE code IN ('core', 'hotel', 'operations')), 107, 0, true),
 
('enterprise', 'Complexe Hôtelier', 'Solution complète tous modules', 'Grandes chaînes', 
 (SELECT ARRAY[id] FROM public.modules WHERE code IN ('core', 'hotel', 'restaurant', 'operations', 'analytics', 'multisite')), 225, 15, true);
