-- Supprimer les dépendances et implémenter la solution corrigée

-- 1. Supprimer le trigger et la fonction existante
DROP TRIGGER IF EXISTS generate_pos_session_number_trigger ON pos_sessions;
DROP FUNCTION IF EXISTS public.generate_pos_session_number() CASCADE;

-- 2. Créer la nouvelle fonction pour générer les numéros de session
...
-- 3. Créer la table pos_sessions si elle n'existe pas
...
-- 4. Fonction d'authentification corrigée
...
-- 5. Fonction de validation simplifiée (sans récursion)
...
-- 6. Fonction utilitaire simplifiée
...
-- 7. Données de test pour debugging
