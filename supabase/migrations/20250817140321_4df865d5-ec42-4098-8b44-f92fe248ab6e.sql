-- Phase 3A FINALE - Élimination totale des défauts de sécurité
...
-- Convertir la vue en fonction sécurisée
...
AS $function$
    SELECT 
        ...
        COALESCE(a.base_price, 0) + COALESCE(SUM(
            CASE 
                WHEN asrv.is_included THEN asrv.quantity * COALESCE(asrv.unit_price, s.unit_price, 0)
                ELSE 0 
            END
