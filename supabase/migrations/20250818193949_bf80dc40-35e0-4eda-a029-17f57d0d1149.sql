-- Corriger la fonction authenticate_pos_user pour utiliser pos_auth_sessions et remplacer gen_random_bytes

-- Supprimer la fonction actuelle avec le problème gen_random_bytes
DROP FUNCTION IF EXISTS public.authenticate_pos_user(text, uuid) CASCADE;

-- Créer la fonction corrigée avec token compatible et bonne table
...
DECLARE
  ...
  v_role text;
BEGIN
  -- Générer un token de session unique (compatible sans gen_random_bytes)
  ...
  -- Insérer ou mettre à jour la session POS (utiliser pos_auth_sessions)
