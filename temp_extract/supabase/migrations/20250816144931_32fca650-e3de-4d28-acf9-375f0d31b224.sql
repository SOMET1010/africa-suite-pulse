...
-- Drop and recreate the room_status_summary view with consistent columns
...
-- These provide the same functionality without the SECURITY DEFINER flag
...
COMMENT ON VIEW public.room_status_summary IS 'Room status summary view with security_barrier enabled for proper RLS enforcement to address linter requirements';