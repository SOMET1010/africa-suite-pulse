-- Phase 1: Security foundations for Users & Permissions
-- This migration adds roles, user security settings (2FA/read-only),
-- global read-only enforcement triggers, auditing, and granular permissions.
...
-- 3) User security settings: 2FA + temporary read-only support
...
-- Policies (view/update own; super_admin full control)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 4) Global read-only enforcement trigger for critical tables
...
-- Attach to critical tables (idempotent drops before creates)
-- Guests
...
-- 5) Auditing: capture changes across critical tables
...
-- Attach audit triggers (drop if exists first)
...
-- 6) Granular permissions powered by profiles and roles
...
BEGIN
  ...
  -- Manager: broad access (backward-compat)
  ...
  -- Otherwise, check profile-bound permissions
...
-- 7) Seed common permissions (insert only if missing)
