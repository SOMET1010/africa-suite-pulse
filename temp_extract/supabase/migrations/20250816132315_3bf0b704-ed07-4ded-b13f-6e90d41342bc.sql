-- Corrected migration to fix Security Definer View warnings
...
CREATE VIEW public.housekeeping_tasks_with_staff
WITH (security_barrier=true)
AS 
SELECT 
    t.id,
...
-- 5. Update the room_status_summary view (keeping original column structure)
DROP VIEW IF EXISTS public.room_status_summary;
CREATE VIEW public.room_status_summary
WITH (security_barrier=true)
AS 
SELECT 
    org_id,
    ...
    -- Keep the original column name and logic
...
-- 6. Document the security approach
...
-- 7. Keep the original SECURITY DEFINER function for backward compatibility in other contexts
-- but mark it clearly
