...
SELECT DISTINCT org_id, 'auto_deduct_stock', '{"enabled": true}', 'inventory', 'Déduction automatique du stock lors des ventes'
FROM public.hotel_settings
WHERE NOT EXISTS (
...
INSERT INTO public.pos_system_settings (org_id, setting_key, setting_value, category, description)
SELECT DISTINCT org_id, 'low_stock_alert', '{"enabled": true, "threshold_days": 7}', 'inventory', 'Alertes de stock faible'
FROM public.hotel_settings
WHERE NOT EXISTS (
  SELECT 1 FROM public.pos_system_settings 
  WHERE pos_system_settings.org_id = hotel_settings.org_id 
...
INSERT INTO public.pos_system_settings (org_id, setting_key, setting_value, category, description)
SELECT DISTINCT org_id, 'kitchen_display_timeout', '{"minutes": 15}', 'kitchen', 'Délai d affichage cuisine'
FROM public.hotel_settings
WHERE NOT EXISTS (
  SELECT 1 FROM public.pos_system_settings 
  WHERE pos_system_settings.org_id = hotel_settings.org_id 
